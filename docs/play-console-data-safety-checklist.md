...
5. ~~**Gemini data retention.**~~ **RESOLVED 2026-05-28** — `GEMINI_API_KEY` is billed under the **paid tier (Pay-as-you-go)**, which per [Gemini API terms](https://ai.google.dev/gemini-api/terms) guarantees no retention beyond processing and no use of prompts to improve Google's models. Privacy policy's "not used to train Google's models" claim stands. Data Safety form: ticket photos processed by Gemini = "data processed ephemerally / not shared with third party for advertising or other purposes beyond the immediate processing".
6. ~~**Ticket photo persistence.**~~ **RESOLVED 2026-05-28** — Cropped JPEG uploaded to Supabase Storage bucket `ticket-photos` at path `{user_id}/{journey_id}_ticket.jpg` (`app/scan/review.tsx` lines 405-447). URL stored in `theater_visits.ticket_image_url` and `user_movies.journey_photos[]`. Original full-res photo is NOT retained. Privacy policy updated 2026-05-28 to reflect this.
7. ~~**AI ticket extraction disclosure.**~~ **RESOLVED 2026-05-28** — Yes, tick the **"App uses generative AI features"** box on the Play Console submission. Declared feature: ticket OCR (Gemini extracts theater name, movie title, showtime from a user-uploaded ticket photo). User-initiated, not auto-generated content, no model outputs presented as creative work. Same disclosure for the App Store privacy section.
8. ~~**Data deletion SLA.**~~ **RESOLVED 2026-05-28** — `supabase/functions/delete-account/index.ts` does immediate synchronous hard delete (no queue, no soft-delete) across 17 tables + auth user. Privacy policy previously claimed "30 days" — **corrected 2026-05-28** to say "immediately" with two narrow exceptions documented (diagnostic data already sent to Sentry/PostHog, and Supabase backup retention up to 30 days). Form answer: **users can request deletion = Yes**, deletion is immediate.
...
- **Open questions:** 8 originally — **all 8 resolved 2026-05-28.** #1 (email), #3 (Facebook), #4 (AdMob Android), #5 (Gemini paid tier), #6 (ticket photo persistence), #7 (AI features box: Yes), #8 (deletion SLA: immediate). #2 (analytics opt-out) resolved by shipping the Settings → Privacy toggles on this branch. This doc is the authoritative submission script.
