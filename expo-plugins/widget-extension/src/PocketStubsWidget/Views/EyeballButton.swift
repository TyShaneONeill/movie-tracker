...
struct EyeballButton: View {
    ...
    // .symbolEffect(.pulse) modifier above.
            ...
            if show.isTrophy {
                ...
                Button(intent: MarkEpisodeWatchedIntent(
                    ...
                    Image(systemName: "eye")
                        ...
                        // Phase 4c.3c: pulse on RPC rejection (unaired or
                        // missing catalog row). .nonRepeating fires one
                        // flash per counter change. Reads per-show counter
                        // from App Group UserDefaults; timeline reload after
                        // the intent triggers body re-eval which re-reads
                        // the counter and drives this symbolEffect.
                        // (Using pulse instead of wiggle because wiggle
                        // requires iOS 18; widget target is iOS 17.)
                        .symbolEffect(.pulse, options: .nonRepeating, value: rejectionCount(for: show.userTvShowId))
