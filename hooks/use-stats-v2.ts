...
export type StatsVariant = 'v1' | 'v2';
...
const ENV_OVERRIDE = process.env.EXPO_PUBLIC_STATS_V2_OVERRIDE;

/**
 * Resolves which Analytics/Stats experience a user should see.
 *
 * Resolution order:
 *   1. `EXPO_PUBLIC_STATS_V2_OVERRIDE` ('true' | 'false') — dev/QA override.
 *   2. The `stats_v2` PostHog flag.
 *
 ...
 *
 * Mirrors `hooks/use-ticket-scan-v2.ts`.
 */
export function useStatsV2(): { variant: StatsVariant; resolving: boolean } {
  const initial = analytics.getFeatureFlag(STATS_V2_FLAG);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const current = analytics.getFeatureFlag(STATS_V2_FLAG);
