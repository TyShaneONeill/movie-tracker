...
/** PostHog flag (id 758095) gating the Lists redesign (profile Lists tab + list detail). */
export const LISTS_V2_FLAG = 'lists_v2';
...
const ENV_OVERRIDE = process.env.EXPO_PUBLIC_LISTS_V2_OVERRIDE;

/**
 * Resolves whether the Lists v2 redesign should render, across every Lists
 * surface (profile Lists tab, list detail: watchlist deck / watching rows /
 * custom backdrop hero).
 *
 * Resolution order:
 *   1. `EXPO_PUBLIC_LISTS_V2_OVERRIDE` ('true' | 'false') — dev/QA override.
 *   2. The `lists_v2` PostHog flag (founder-only at launch).
 *
 * Fails CLOSED: while the flag is still loading (value `undefined`) `enabled`
 * is false, so an unresolved flag always renders the legacy Lists surfaces.
 * `resolving` lets the caller hold a neutral frame instead of flashing legacy
 * then snapping to v2 for a tester. Mirrors `hooks/use-first-takes-v2.ts`.
 */
export function useListsV2(): { enabled: boolean; resolving: boolean } {
  const initial = analytics.getFeatureFlag(LISTS_V2_FLAG);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const current = analytics.getFeatureFlag(LISTS_V2_FLAG);
