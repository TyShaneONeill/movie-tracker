...
export const RELEASE_CALENDAR_V2_FLAG = 'release_calendar_v2';

const RESOLVE_TIMEOUT_MS = 1200; // give PostHog a beat to load flags before falling back
const POLL_INTERVAL_MS = 250;

export type ReleaseCalendarVariant = 'v1' | 'v2';
...
const ENV_OVERRIDE = process.env.EXPO_PUBLIC_RELEASE_CALENDAR_V2_OVERRIDE;

/**
 * Resolves which Release Calendar experience a user should see.
 *
 * Resolution order:
 *   1. `EXPO_PUBLIC_RELEASE_CALENDAR_V2_OVERRIDE` ('true' | 'false') — dev/QA override.
 *   2. The `release_calendar_v2` PostHog flag.
 *
 ...
 *
 * Mirrors `hooks/use-stats-v2.ts`.
 */
export function useReleaseCalendarV2(): { variant: ReleaseCalendarVariant; resolving: boolean } {
  const initial = analytics.getFeatureFlag(RELEASE_CALENDAR_V2_FLAG);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const current = analytics.getFeatureFlag(RELEASE_CALENDAR_V2_FLAG);
