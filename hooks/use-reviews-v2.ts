...
/** PostHog flag (id 757570) gating the Reviews profile-tab redesign. */
export const REVIEWS_V2_FLAG = 'reviews_v2';
...
const ENV_OVERRIDE = process.env.EXPO_PUBLIC_REVIEWS_V2_OVERRIDE;

/**
 * Resolves whether the Reviews v2 tab should render, on BOTH profile screens
 * (own + user/[id]).
 *
 * Resolution order:
 *   1. `EXPO_PUBLIC_REVIEWS_V2_OVERRIDE` ('true' | 'false') — dev/QA override.
 *   2. The `reviews_v2` PostHog flag (founder-only at launch).
 *
 ...
 */
export function useReviewsV2(): { enabled: boolean; resolving: boolean } {
  const initial = analytics.getFeatureFlag(REVIEWS_V2_FLAG);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const current = analytics.getFeatureFlag(REVIEWS_V2_FLAG);
