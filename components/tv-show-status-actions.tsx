...
interface TvShowStatusActionsProps {
  ...
  onStatusChange: (status: TvShowStatus | null) => void;
}

// SVG icon components
function BookmarkIcon({ color, size = 24 }: { color: string; size?: number }) {
  return (
...
function XCircleIcon({ color, size = 24 }: { color: string; size?: number }) {
  return (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
...
export function TvShowStatusActions({
  ...
  return (
    <View style={styles.container}>
      ...
      <StatusButton
        label="ON HOLD"
        icon={<PauseIcon color={inactiveColor} />}
        activeIcon={<PauseIcon color={onHoldColor} />}
        isActive={isOnHold}
      ...
      <StatusButton
        label="DROPPED"
        icon={<XCircleIcon color={inactiveColor} />}
        activeIcon={<XCircleIcon color={droppedColor} />}
        isActive={isDropped}
