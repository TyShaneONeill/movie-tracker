...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: journeys, error: fetchError } = await supabaseAdmin.from('user_movies').select('id, user_id, ai_poster_url').like('ai_poster_url', 'data:image%');
    if (fetchError) {
    ...
    for (const journey of journeys){
      try {
        ...
        // Upload to storage
        const { error: uploadError } = await supabaseAdmin.storage.from('journey-art').upload(filePath, imageBytes, {
        ...
        // Get public URL
        const { data: urlData } = supabaseAdmin.storage.from('journey-art').getPublicUrl(filePath);
        // Update database with storage URL
        const { error: updateError } = await supabaseAdmin.from('user_movies').update({
          ai_poster_url: urlData.publicUrl
        }).eq('id', journey.id);
        if (updateError) {
