...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Fetch friends' reviews for this movie (public + followers_only)
    ...
    if (queryError) {
      console.error('[get-friends-ratings] Query error:', queryError);
      throw new Error('Failed to fetch friends ratings');
    }

    const friendsRatings: FriendRatingResponse[] = (rows as FriendRatingRow[] || []).map((row) => ({
      userId: row.user_id,
      rating: row.rating,
    ...
    // Compute average rating from friends who gave a numeric rating
    const numericRatings = friendsRatings
      .map((r) => r.rating)
