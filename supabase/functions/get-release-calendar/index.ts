...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { month, year, region = 'US' } = await req.json();
    ...
    const { data: cachedEntries } = await supabase.from('release_date_cache').select('tmdb_id').eq('region', region).gte('release_date', startDate).lte('release_date', endDate).gte('fetched_at', cacheThreshold).limit(1);
    ...
    if (!hasFreshCache) {
      ...
      for(let i = 0; i < allMovies.length; i += BATCH_SIZE){
        ...
        for (const result of results){
          if (!result) continue;
          const countryData = result.results.find((r)=>r.iso_3166_1 === region);
        ...
        if (rows.length > 0) {
          const { error: upsertError } = await supabase.from('release_date_cache').upsert(rows, {
    ...
    const { data: releases, error: readError } = await supabase.from('release_date_cache').select('tmdb_id, release_type, release_date, certification').eq('region', region).gte('release_date', startDate).lte('release_date', endDate).order('release_date', {
      ascending: true
    });
    if (readError) throw readError;
    // If we used cache and don't have movieMap yet, fetch movie info
    const tmdbIds = [
      ...new Set((releases || []).map((r)=>r.tmdb_id))
    ];
    if (movieMap.size === 0 && tmdbIds.length > 0) {
      // Try the movies cache table first
      const { data: cachedMovies } = await supabase.from('movies').select('tmdb_id, title, poster_path, backdrop_path, genre_ids, vote_average').in('tmdb_id', tmdbIds);
      if (cachedMovies) {
